...
const applyFEN = fen => {
    const board8 = toBoard8(fromFEN(fen)).reverse();
...
const _applyBoard = originalBoard => {
    let board = originalBoard.reverse();
...
const showMove = (pIndex, mIndex) => {
    board.children[63 - pIndex].style.backgroundColor = '#beffad';
    board.children[63 - mIndex].style.backgroundColor = '#96ff7a';
}
...
const main = () => {
        ...
        if (getTurn(fen) == 'b') {
            if (isNaN(clickedPieceIndex)) {
                if (pieceColor(toBoard8(fromFEN(fen)), sIndex) == BLACK) {
                    ...
                    board.children[63 - sIndex].style.backgroundColor = '#54ed1c';
                    for (const [pIndex, mIndexes] of getMoves(fromFEN(fen)).bMoves) {
                        if (toIndex8(pIndex) == sIndex) {
                            for (mIndex of mIndexes) {
                                board.children[63 - toIndex8(mIndex)].style.backgroundColor = '#1cdfed';
                            }
                        }
                    }
                }
    ...
    overlayArr.map((square, sIndex) => {
        square.onclick = e => squareClick(e, 63 - sIndex);
    });
